function [PC_preds, idrem2] = pc_model(PCs,Features, test_features, varargin)
...
test_preds = [test_features.xco2(:,:),test_features.delta_xco2(:,:),test_features.delta_solzen(:,:),test_features.solzen(:,:), test_features.azim(:,:), test_features.temp(:,:), test_features.delta_temp(:,:)...
    ...
    test_features.delta_azim(:,:), test_features.delta_pressure(:,:), test_features.delta_wind_speed(:,:), test_features.delta_airmass(:,:), test_features.delta_xh2o(:,:), test_features.prior_diff(:,1),test_features.detrended_xco2(:,:)]; %, , Features.temp(:,:)Features.trop_alt(:,:), Features.mid_trop_pot_temp(:,:), Features.pres_alt(:,:), ...
...
%py.sklearn.tree.plot_tree(tree)
 ...
 %Variance = py.forestci.random_forest_error(MDL, x,a);
  ...
  for i = 1:length(name_list)
    ...
    PC_preds.(name_list{i})(ind).inBagStats = r2rmse(yhat(:,i),y(:,i));
    PC_preds.(name_list{i})(ind).oobStats = r2rmse(y1_oob(:,i),y(:,i));
