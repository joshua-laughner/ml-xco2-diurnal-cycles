...
rem = unique([find(isnan(X)); find(isnan(Y))]);
X(rem) = [];
Y(rem) = [];
x = X;
y = Y;
if nargin > 2
    okargs = {'smoothing','bins','plottype','log','contourFlag','marker','msize','filled'};
    for j=1:2:nargin-2
        pname = varargin{j};
        pval = varargin{j+1};
...
[dum(:,2),bin(:,2)] = histc(X,edges1);
[dum(:,1),bin(:,1)] = histc(Y,edges2);
bin(bin==0) = 1;
