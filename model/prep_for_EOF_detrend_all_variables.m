function [big_quart_hour_av, big_quart_hour_hours,big_location_struct] = prep_for_EOF_detrend_all_variables(big_location_struct,badmonths_struct,varargin)
variables = varargin;
...
for loc = 1:length(locations)
    ...
    for i = 1:length(variables)
        var_struct.(variables{i}) = nan(days,27);
    end
    ...
    % this section is the direct quarter hour averaging
        ...
        for j = 1:length(variables)
            var_struct.(variables{j})(day,i) = mean(location_struct.(variables{j})(timeind, day),'omitnan');
        end
...
for day = 1:days
   ...
   continue
    ...
    if ismember(months(day),badmonths_struct.(locations{loc})) %flagging for not growing season, the cutoffs are given
    acceptable_day(day) = 0;
    continue
    end
...
for j = 1:length(variables)
   var_struct.(variables{j})(tossers,:) = []; 
end
...
for day = 1:length(quart_hour_av_xco2(:,1))
    ...
    for j = 1:length(variables)
        var_struct.(variables{j})(day,:)  = interp1(steps(nnanind), var_struct.(variables{j})(day,nnanind), steps, 'spline', 'extrap');
    end
end
...
big_quart_hour_av.(locations{loc}).xco2 = quart_hour_av_xco2;
for j = 1:length(variables)
    big_quart_hour_av.(locations{loc}).(variables{j}) = var_struct.(variables{j});
end
